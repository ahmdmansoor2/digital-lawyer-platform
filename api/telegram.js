...
const KEYBOARD = {
  keyboard: [
    ...
    [{ text: '🖼️ نشر صورة فيسبوك نانو بنانا' }, { text: '🎬 توليد ريلز وشورتس' }],
...
- إذا طلب تعديلاً برمجياً في كود المنصة: وضح له أن التعديلات البرمجية المباشرة في ملفات المشروع تنفذ من بيئة التطوير Antigravity على جهازه، وأن بإمكانه هنا تشغيل النشر والفحص والاستشارات الحية فوراً.`;
...
export default async function handler(req, res) {
  ...
  if (text.includes('فيس') || text.includes('بوست') || text.includes('صورة') || text.includes('نانو بنانا') || text === '🖼️ نشر صورة فيسبوك نانو بنانا' || text === '/facebook') {
    const ok = await triggerWorkflow('daily-cards.yml');
    if (ok) {
      await sendTelegram('🖼️ <b>تم تشغيل ماكينة فيسبوك بنظام نانو بنانا برو!</b>\nجاري الآن توليد صورة سينمائية 3D معبرة بدقة عن الموضوع القانوني ونشرها على الصفحة فوراً.');
    } else {
      await sendTelegram('⚠️ تعذر تشغيل سير عمل فيسبوك، تأكد من ضبط GITHUB_TOKEN في المتغيرات.');
    }
    return res.status(200).send('OK');
  }
