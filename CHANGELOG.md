...
## [v2.9.12] — 2026-09-03 (🎨 إصلاح جذري لمشكلة الثيم الداكن المُجبَر)

### المشكلة
- `useEffect` في `App.tsx` كان يُجبر الثيم `dark` عند كل تحميل ويُضيف `class="dark"` لـ `<html>`.
- المستخدم لم يكن يستطيع تغيير الثيم حتى لو اختاره من الإعدادات.

### الحل
- **حُذف** الـ useEffect المُجبِر من `App.tsx`.
- **إضافة migration** يُزيل `'dark'` المُخزَّن في localStorage عند أول تحميل.
- الثيم الافتراضي عاد لـ `'slate'` (فاتح كما كان في التصميم الأصلي).
