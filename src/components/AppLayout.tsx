...
export default function AppLayout(props: AppLayoutProps) {
  ...
  return (
    <div className={`theme-${appTheme} min-h-screen flex flex-col md:flex-row text-end font-sans transition-colors duration-200 ${getThemeBgClass(appTheme)}`} dir="rtl" id="lawfirm-app-root">
      ...
      <header className={`md:hidden sticky top-0 z-30 px-3.5 py-2.5 flex items-center justify-between border-b backdrop-blur-xl shrink-0 ${isLightWorkspace ? 'bg-white/95 border-slate-200 text-slate-800 shadow-sm' : 'bg-slate-950/95 border-slate-800/80 text-white shadow-md'}`}>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className={`p-2 rounded-xl border transition cursor-pointer ${isLightWorkspace ? 'bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200' : 'bg-slate-900 border-slate-700 text-slate-200 hover:bg-slate-800'}`}
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-black text-xs">المحامي الرقمي</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
          ...
          {onShowInfoCenter && (
            <button
              ...
              className="px-2.5 py-1 rounded-xl bg-indigo-600/20 border border-indigo-500/40 text-indigo-300 text-[10.5px] font-bold hover:bg-indigo-600/30 transition cursor-pointer"
            >
              الرئيسية
            </button>
          )}
