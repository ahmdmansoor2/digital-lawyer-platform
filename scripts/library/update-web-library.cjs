  ...
  <script>
    ...
    window.openBookModal = function(bookJsonStr) {
      try {
        ...
        document.getElementById('modalBookDesc').innerText = 'المؤلف: ' + b.author + ' | الفرع: ' + b.branchName + ' | الحجم: ' + b.sizeFormatted + ' (' + b.ext.toUpperCase() + ')';
        // الرابط المباشر
        btnDownload.href = '/data/download?file=' + encodeURIComponent(b.relPath);
        btnDownload.setAttribute('download', b.originalName);
        modal.classList.add('open');
      } catch(e) {}
    };
