  ...
  return new Promise((resolve, reject) => {
    ...
    const attachCDP = () => {
      ...
      for (const version of versions) {
        try {
          authWin.webContents.debugger.attach(version);
          return true;
        } catch (err) {
          console.warn(`[GoogleAuth] debugger.attach(${version}) failed:`, err?.message || err);
        }
      }
      return false;
    };

    const cdpEnabled = attachCDP();
    if (!cdpEnabled) {
      console.warn('[GoogleAuth] CDP debugger unavailable; aborting native Electron Google login.');
      fail(new Error('CDP unavailable'));
      return;
    }
      ...
      if (method === 'Network.loadingFinished' && params.requestId === currentRequestId) {
        ...
        authWin.webContents.debugger.sendCommand('Network.getResponseBody', {
          ...
          try {
            ...
            // Keep waiting until we get a valid auth response payload.
          }
        }).catch(() => {});
      }
    });
...
function createWindow() {
  ...
  // ونطبّق CSP فقط على صفحات التطبيق المحليّة (file:// / localhost)
  // حتى لا نكسر عمليات OAuth/Google التي تعرض صفحات بعيدة الأصل.
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    const url = details.url || '';
    const isLocalAppUrl = url.startsWith('file://') || url.startsWith('app://') || url.startsWith('http://localhost');
    if (!isLocalAppUrl) {
      return callback({ responseHeaders: details.responseHeaders });
    }

    const devCSP = isDev
      ? "default-src 'self' blob: data: file: http://localhost:3000 ws:; " +
        "script-src 'self' 'unsafe-inline' blob: https://fonts.googleapis.com https://fonts.gstatic.com http://localhost:3000; " +
        "script-src-elem 'self' 'unsafe-inline' blob: https://fonts.googleapis.com https://fonts.gstatic.com http://localhost:3000; " +
        "style-src 'self' 'unsafe-inline' blob: data: https://fonts.googleapis.com http://localhost:3000; " +
        "style-src-elem 'self' 'unsafe-inline' blob: data: https://fonts.googleapis.com https://fonts.gstatic.com http://localhost:3000; " +
        "img-src 'self' blob: data: file: https://fonts.gstatic.com https://lh3.googleusercontent.com https://www.gstatic.com http://localhost:3000; " +
        "media-src 'self' blob: data: file:; " +
        "frame-src 'self' blob: data: file:; " +
        "connect-src 'self' blob: data: http://localhost:3000 ws: https://fonts.googleapis.com https://fonts.gstatic.com https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://firestore.googleapis.com https://www.googleapis.com https://www.gstatic.com https://accounts.google.com https://apis.google.com https://firebasestorage.googleapis.com https://storage.googleapis.com; " +
        "font-src 'self' blob: data: https://fonts.gstatic.com;"
      : "default-src 'self' blob: data: file:; " +
        "script-src 'self' blob: https://fonts.googleapis.com https://fonts.gstatic.com; " +
        "script-src-elem 'self' blob: https://fonts.googleapis.com https://fonts.gstatic.com; " +
        ...
        "img-src 'self' blob: data: file: https://fonts.gstatic.com https://lh3.googleusercontent.com https://www.gstatic.com; " +
        "media-src 'self' blob: data: file:; " +
        "frame-src 'self' blob: data: file:; " +
        "connect-src 'self' blob: data: https://fonts.googleapis.com https://fonts.gstatic.com https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://firestore.googleapis.com https://www.googleapis.com https://www.gstatic.com https://accounts.google.com https://apis.google.com https://firebasestorage.googleapis.com https://storage.googleapis.com; " +
        "font-src 'self' blob: data: https://fonts.gstatic.com;";
